...
If you're setting this up fresh (no `schema.sql` run yet), just run all three in order — no separate migration step needed.
...
- **Real menu data** — catalog is empty until items are added via `/admin/items.html`, or seeded from the extracted PowerPoint data.
